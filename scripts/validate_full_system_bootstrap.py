...
def validate_docs_and_generated() -> None:
    ...
    for path in [RUNBOOK, GENERATED_RUNBOOK]:
        ...
        require_contains(path, "Fresh Agent First Reply Contract", "fresh-agent first reply contract section")
        require_contains(path, "profile/workspace boundary", "fresh-agent profile/workspace boundary marker")
        require_contains(path, "Stage 8 local pilot gate", "fresh-agent Stage 8 marker")
        require_contains(path, "domain-specific item intake such as equipment photos", "fresh-agent domain-specific intake guard")
