...
$stage4Lines = @(
  ...
  '    starter_skills: [launchroom-tool-readiness-smoke, codex, claude-code, github-pr-workflow]',
  ...
  '    starter_skills: [hermes-agent, experience-grounded-work-preflight, governed-agent-engineering-standards]',
...
$agentReadinessStartLines = @(
  ...
  '8. TOOLCHAIN_ACTIVATION_PLAN.yaml — machine-readable gated activation plan.',
  '9. EXECUTION_READINESS_REPORT.yaml — machine-readable status.',
  '',
  '## Rule',
  '',
...
$toolchainRequirementsLines = @(
  ...
  '| External agent handoff | git, gh, codex/claude optional | terminal, file, skills, delegation optional | launchroom-tool-readiness-smoke, codex, claude-code | external_agent_gate | smoke pass or blocker recorded |',
  ...
  '',
  'Machine-readable version: TOOLCHAIN_ACTIVATION_PLAN.yaml.',
  '',
...
$skillLoadPlanLines = @(
  ...
  '- launchroom-tool-readiness-smoke: before Codex/Claude/gh external handoff.',
...
$toolchainActivationPlanLines = @(
  ...
  '  stage8_file_change_plan: .hermes/local-pilot/FILE_CHANGE_PLAN.md',
  ...
  '  stage3_software_purpose_map: .hermes/reports/software-purpose-map.yaml',
  '  stage3_install_recommendation: .hermes/reports/software-install-recommendation.yaml',
  '  stage4_capability_graph: .hermes/reports/capability-graph.yaml',
  '  stage4_starter_capability_pack: .hermes/reports/starter-capability-pack.yaml',
  'toolchain_requirements:',
  '  - id: local_validation_and_repo_flow',
  '    work_class: local validation and governed repo flow',
  '    required_software:',
  '      - git',
  '      - python',
  '    hermes_toolsets:',
  ...
  '      - requesting-code-review',
  '      - github-pr-workflow',
  '    gate: implementation_gate_before_file_changes',
  '    verification: readback plus validators plus independent review',
  '  - id: web_or_node_slice',
  '    work_class: frontend or web SaaS slice',
  '    required_software:',
  '      - nodejs_lts',
  '      - npm',
  '    hermes_toolsets:',
  ...
  '      - terminal',
  '      - code_execution',
  '      - browser_optional_after_local_server_gate',
  '    skills:',
  '      - test-driven-development',
  '      - systematic-debugging',
  '      - requesting-code-review',
  '    gate: install_gate_if_node_missing_then_implementation_gate',
  '    verification: package-script-derived tests/builds after gate',
  '  - id: external_agent_handoff',
  '    work_class: Codex Claude or external agent packet handoff',
  '    required_software:',
  '      - git',
  '      - gh_optional',
  '      - external_agent_cli_optional',
  '    hermes_toolsets:',
  '      - terminal',
  '      - file',
  '      - skills',
  ...
  '    skills:',
  '      - launchroom-tool-readiness-smoke',
  '      - governed-agent-executor-loop',
  '    gate: external_agent_gate',
  '    verification: smoke pass or blocker recorded before handoff',
  'software_gap_analysis:',
  '  present_or_required_baseline:',
  '    - hermes',
  '    - git',
  '    - python',
  '  gated_install_candidates:',
  '    - nodejs_lts_npm',
  '    - ripgrep',
  '    - uv',
  '    - github_cli_gh',
  '    - docker_desktop_optional_high_risk',
  '  source_evidence_missing:'
)
if ($missingAgentReadinessEvidence.Count -eq 0) {
  $toolchainActivationPlanLines += '    - none'
} else {
  foreach ($missingEvidence in $missingAgentReadinessEvidence) { $toolchainActivationPlanLines += "    - $(ConvertTo-YamlSingleQuotedScalar $missingEvidence)" }
}
$toolchainActivationPlanLines += @(
  'toolset_plan:',
  '  - phase: planning_and_repair',
  ...
  '      - skills',
  '      - todo',
  '    gate: none_for_current_enabled_session_tools_but_no_new_toolset_activation',
  '  - phase: implementation_after_gate',
  '    recommended_toolsets:',
  '      - file',
  '      - terminal',
  '      - code_execution',
  ...
  '  - phase: ui_verification_after_local_server_gate',
  '    recommended_toolsets:',
  '      - browser',
  '      - computer_use',
  '      - vision',
  '    gate: local_server_or_ui_verification_gate',
  'skill_load_plan:',
  '  - phase: preflight',
  '    skills:',
  '      - experience-grounded-work-preflight',
  '      - governed-agent-engineering-standards',
  '    gate: load_existing_skill_only',
  ...
  '    gate: git_publication_or_release_gate_as_applicable',
  'agent_pipeline:',
  '  - role: planner_repair_agent',
  '    starts_after: Stage 9 blocker review',
  '    allowed_actions: plan and repair packet only',
  '    gate: owner_repair_or_acceptance_gate',
  '  - role: toolchain_verifier',
  '    starts_after: Stage 10 readiness acceptance',
  '    allowed_actions: read-only version and command readiness checks',
  '    gate: toolchain_verification_gate',
  '  - role: implementer',
  '    starts_after: implementation gate',
  '    allowed_actions: approved file scope only',
  '    gate: implementation_gate',
  '  - role: verification_arbiter',
  '    starts_after: implementation evidence exists',
  '    allowed_actions: approved validators/tests/evidence review',
  '    gate: verification_gate',
  'install_plan_entries:',
  '  - software: Node.js LTS + npm',
  '    why: frontend or web SaaS slice may need package scripts',
  '    command_shape: winget install OpenJS.NodeJS.LTS',
  '    verification_commands:',
  '      - node --version',
  '      - npm --version',
  '    risk: PATH restart or system package change',
  '    rollback: winget uninstall OpenJS.NodeJS.LTS',
  '    gate: install_gate',
  '  - software: ripgrep',
  '    why: fast repo search and drift inspection',
  '    command_shape: winget install BurntSushi.ripgrep.MSVC',
  '    verification_commands:',
  '      - rg --version',
  '    risk: PATH or system package change',
  '    rollback: winget uninstall BurntSushi.ripgrep.MSVC',
  '    gate: install_gate',
  '  - software: uv',
  '    why: Python dependency and tool runner for Python slices',
  '    command_shape: winget install astral-sh.uv',
  '    verification_commands:',
  '      - uv --version',
  '    risk: PATH or system package change',
  '    rollback: winget uninstall astral-sh.uv',
  '    gate: install_gate',
  '  - software: GitHub CLI gh',
  '    why: PR CI and release workflow after publication gate',
  '    command_shape: winget install GitHub.cli',
  '    verification_commands:',
  '      - gh --version',
  '      - gh auth status',
  '    risk: auth flow may be needed; no token in chat',
  '    rollback: winget uninstall GitHub.cli',
  '    gate: install_or_auth_gate',
  'command_readiness:',
  '  read_only_allowed:',
  '    - inspect approved project files',
  '    - git status --short --branch',
  '    - version checks for already installed tools',
  '    - read generated Stage 3 through Stage 10 reports',
  '  gated_local_commands:',
  '    - package-manager test or build commands after implementation gate',
  '    - format or lint commands after file scope approval',
  '  install_commands:',
  '    - install commands from install_plan_entries only after install_gate',
  '  forbidden_without_gate:',
  '    - commands that read or print secrets',
  '    - git push merge rebase reset clean tag release publication',
  '    - cloud provider runtime deploy or mutation',
  '    - gateway pairing autostart or live messaging setup',
  '    - n8n MCP database mutation',
  'readiness_blockers:',
  '  - id: stage9_unresolved_findings',
  '    source: .hermes/project-audit/AUDIT_FINDINGS.yaml',
  '    blocks_execution: true',
  '    next_action: repair findings or owner accepts readiness-analysis-only continuation',
  '  - id: owner_readiness_acceptance_missing',
  '    source: Stage 10 owner decision',
  '    blocks_execution: true',
  '    next_action: owner accepts readiness package before implementation gate',
  '  - id: implementation_gate_missing',
  '    source: owner implementation gate',
  '    blocks_execution: true',
  '    next_action: separate implementation gate after readiness acceptance',
  'action_flags:',
...
Write-Utf8NoBom (Join-Path $agentReadinessRoot 'TOOLCHAIN_ACTIVATION_PLAN.yaml') ($toolchainActivationPlanLines -join "`n")
...
$executionReadinessLines = @(
  ...
  '  stage3_install_recommendation: .hermes/reports/software-install-recommendation.yaml',
  '  stage4_starter_capability_pack: .hermes/reports/starter-capability-pack.yaml',
  '  stage10_activation_plan: .hermes/agent-readiness/TOOLCHAIN_ACTIVATION_PLAN.yaml',
  'generated_files:',
  '  - START_HERE.md',
...
$matrixLines = @(
  ...
  '| Stage 3 tool readiness | launchroom-tool-readiness-smoke, hermes-agent | windows-desktop-agent-setup | before tool probes |',
  '| Stage 4 capability pack | governed-agent-engineering-standards | launchroom-positive-result-capture | before capability mapping |',
  ...
  '| Stage 10 agent execution readiness | governed-agent-engineering-standards, launchroom-tool-readiness-smoke | codex, claude-code, github-pr-workflow | before implementation gate |',
  '| Stage 11 workspace hygiene | governed-agent-engineering-standards | launchroom-positive-result-capture | before cleanup/archive decisions |',
  '| Stage 12 skill capture | hermes-agent-skill-authoring, governed-agent-engineering-standards | launchroom-positive-result-capture | before candidate creation/promotion |',
  '',
...
$optionalLines = @(
  ...
  '| launchroom-tool-readiness-smoke | Codex/Claude/external agent handoff | external-agent gate |',
  '| governed-messaging-gateway-setup | Telegram/Slack/email/gateway setup | gateway/runtime gate |',
  '| launchroom-positive-result-capture | successful repeatable workflow capture | capture gate |'
)
...
Write-Host "agent_execution_readiness: toolchain requirements -> software gap analysis -> Hermes toolset plan -> skill load plan -> agent pipeline plan -> install plan -> command readiness -> structured activation plan"
